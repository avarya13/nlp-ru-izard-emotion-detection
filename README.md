...
This project addresses multi‑label emotion recognition in Russian‑language texts. The **RuIzardEmotions** dataset — comprising 30,000 Reddit comments annotated with ten emotion categories — is used for training and evaluation. The baseline `ruBERT‑tiny2` model is reproduced from the [authors' repository](https://github.com/Djacon/russian-emotion-detection), and several enhancements are explored. In particular, we replace the SoftMax activation with Sigmoid at the output layer, which is more suitable for multi‑label classification.
...
Choose one of the available models: `ruroberta_large`, `rubert_tiny2`, `rubert_base`, `rubert-base-cased-conv`.  
Example:
...
Monitor metrcis during model training using `MLFlow`:
```bash
